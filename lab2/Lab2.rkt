...
(define pass1 (make-boarding-pass (make-traveller "John" "Doe") "Seattle" "Boston" (make-gate "A" 34) (make-time 22 30)))
(define pass2 (make-boarding-pass (make-traveller "John" "Doe") "Seattle" "Boston" (make-gate "A" 34) (make-time 22 30)))
(define pass3 (make-boarding-pass (make-traveller "James" "Doe") "Chicago" "Boston" (make-gate "C" 34) (make-time 21 30)))
...
(define (print-pass b-pass)
  (string-append (traveller-first (boarding-pass-traveller b-pass)) " "
                 ...
                 (boarding-pass-source-city b-pass) " -> "
                 (boarding-pass-destination-city b-pass) ", Gate "
                 (gate-prefix (boarding-pass-gate b-pass))
                 (number->string (gate-suffix (boarding-pass-gate b-pass))) " @ "
                 (number->string (time-hours (boarding-pass-time b-pass))) ":"
                 (number->string (time-minutes (boarding-pass-time b-pass)))))


;; Boarding-Pass=?: Boarding-Pass Boarding-Pass -> Boolean
;; Checks to see if two boarding passes have the same data
(check-expect (boarding-pass=? pass1 pass2) #true)
(check-expect (boarding-pass=? pass1 pass3) #false)
(define (boarding-pass=? pass1 pass2)
  (if (string=? (print-pass pass1)
                (print-pass pass2))
      #true
      #false))